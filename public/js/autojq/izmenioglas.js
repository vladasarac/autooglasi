...
//ovde su hendleri za vju izmenioglas.blade.php
//----------------------------------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------------------------------
  
  //posto tabela na vrhu izmenioglas.blade.php na malim ekranima lose izgleda ovde merimo ekran i ako se ispostavi da je mali uklanjamo tu tabelu
  ...
  if(widthekrana < 10.5 || /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){
    ...
    //generisemo div koji prikazuje osnovne podatke korisnika
    var podatciuseraout = '';
...
//---------------------------------------------------------------------------------------------------------------------------------------
  ...
  //podesavanje velicine slike u izmenioglas.blade.php
  var width = $('.izmenioglasslike').width();
  var height = width / 1.82;
  //alert(height);
  $('.izmenioglasslike').css('height', height);  

//---------------------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------------
   
  //brisanje jedne slike u izmenioglas.blade.php tj klik na trash ikonicu .deleteizmenioglasimg iznad dodate slike
  $('.deleteizmenioglasimg').on('click', function(){
    var idslike = $(this).attr('idslike');
    //alert(userverification+'/'+oglasid+'/'+idslike+'.jpg');
    $.ajax({ //saljemo metodu id marke , izvadimodelurl je definisan na dnu vjua to je ruta '/izvadimodele'
      method: 'POST',
      url: obrisislikuurl,
      data: { idslike: idslike, userverification: userverification, oglasid: oglasid, _token: token }
    }) // kad obrisisliku( ) OglasControllera vrati odgovor
    .done(function(o){
      console.log(o);
      if(o.delete == 1){ // ako je brisanje slike uspelo
        //uklanjamo div .card #card+idslike, prvo fadeout pa remove
        $('#card'+o.idslike).fadeOut(300, function(){ 
          $(this).remove(); 
        });
        //pravimo input za upload slike kom ce name biti slika + id tj. redni br obrisane slike
        var out = '<div class="col-md-2">';
        out += '<div class="image-upload">';
        out += '<label for="slika'+o.idslike+'" class="slikaupload">';
        out += '<b>Slika '+o.idslike+'</b><br>';
        out += '<img style="width: 150px; height: 80px;" id="slika'+o.idslike+'holder" src="'+slikeurl+'/img/kolasilueta2.png"/>';
        out += '</label>';
        out += '<input id="slika'+o.idslike+'" name="slika'+o.idslike+'" type="file"/>';
        out += '</div>';
        out += '</div>';
        //ako vec ima 6 inputa za slike novonapravljeni input za upload ubacujemo na kraj diva .redzaslike2
        if(mds > 6){
          $(out).appendTo('.redzaslike2');
        //ako je ovo 6 po redu input za upload slike prvo pravimo div .row .redzaslike2 i u njega ubacujemo novi input za upload slike 
        }else if(dodatihinputazaslike == 6){
          var out1 = '<div class="row redzaslike2">'+out+'</div>';
          $(out1).insertAfter('.redzaslike1');
        //ako je broj inputa za upload slike < 6 onda novonapravljeni input za upload ubacujemo na kraj diva row .redzaslike1  
        }else{
          $(out).appendTo('.redzaslike1');
        }
        //povecavamo ovu var za 1 da bi ako bude jos brisanja slika hendler znao koliko slika ima a koliko polja za upload
        dodatihinputazaslike++;
        //ovde se menja vrednost u b .brojdodatihslika koji prikazuje useru koliko ima dodatih slika uz oglas
        if(brslika == 12){
          brslika--;
          var out2 = '<h3 class="text-danger">Do sada dodatih slika - <b class="brojdodatihslika">'+brslika+'</b></h3>';
          $('.maxslika').remove();
          $(out2).insertBefore('.prikazdodatihslika');
        }else{
          brslika--;
          $('.brojdodatihslika').html(brslika);
        }
        //menjamo vrednost u b #mds koji prikazuje useru koliko slika jos moze da uploaduje
        mds++
        $('#mds').html(mds);
        //alert(dodatihinputazaslike);
      }else{
...
//---------------------------------------------------------------------------------------------------------------------------------------
  ...
  $('body').on('change', '#slika1', function(){
    holder = 'slika1holder';
    readURL(this);
  });
  $('body').on('change', '#slika2', function(){
    holder = 'slika2holder';
    readURL(this);
  });
  $('body').on('change', '#slika3', function(){
    holder = 'slika3holder';
    readURL(this);
  });
  $('body').on('change', '#slika4', function(){
    holder = 'slika4holder';
    readURL(this);
  });
  $('body').on('change', '#slika5', function(){
    holder = 'slika5holder';
    readURL(this);
  });
  $('body').on('change', '#slika6', function(){
    holder = 'slika6holder';
    readURL(this);
  });  
  $('body').on('change', '#slika7', function(){
    holder = 'slika7holder';
    readURL(this);
  });
  $('body').on('change', '#slika8', function(){
    holder = 'slika8holder';
    readURL(this);
  });
  $('body').on('change', '#slika9', function(){
    holder = 'slika9holder';
    readURL(this);
  });
  $('body').on('change', '#slika10', function(){
    holder = 'slika10holder';
    readURL(this);
  });
  $('body').on('change', '#slika11', function(){
    holder = 'slika11holder';
    readURL(this);
  });
  $('body').on('change', '#slika12', function(){
...
//---------------------------------------------------------------------------------------------------------------------------------------
//provera da li su obavezna polja u formi popunjena tj VALIDACIJA FORME ZA IZMENU OGLASA
//---------------------------------------------------------------------------------------------------------------------------------------

  //hendler za btn za submit forme iz izmenioglas.blade.php
